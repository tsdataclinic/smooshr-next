...
from server.api.views import app, azure_scheme, get_session
...
class MockAzureUser:
    def __init__(self, claims: dict[str, str | list[str]]):
        self.claims: dict[str, str | list[str]] = claims
...
def setup_db():
    """Create and configure a new in-memory SQLite database for testing"""
...
@pytest.fixture(scope="function")
def empty_db(setup_db: Session):
    """Create and configure a new in-memory SQLite database for testing.
    This is the fixture for an empty DB."""
    yield setup_db


@pytest.fixture(scope="function")
def db_with_user(setup_db: Session):
    """Create and configure a new in-memory SQLite database for testing.
...
def test_get_self_user(db_with_user: Session):
...
def test_create_new_user(empty_db: Session):
    response = client.get("/api/users/self")
    assert response.status_code == 200
    data = response.json()
...
def test_create_workflow(db_with_user: Session):
    response = client.post("/api/workflows/", json={"title": "Test Workflow"})
    assert response.status_code == 200
    data = response.json()
...
def test_get_workflow(db_with_user: Session):
...
def test_fetch_workflow_with_incorrect_user(db_with_user: Session):
    client = TestClient(app)
...
def test_get_workflows(db_with_user: Session):
    test_workflow = DBWorkflow(
        title="Test Workflow",
        owner=MOCK_USER_ID,
    )
...
def test_delete_workflow(db_with_user: Session):
    test_workflow = DBWorkflow(
        title="Test Workflow",
        owner=MOCK_USER_ID,
    )
    ...
    workflow_in_db = (
        db_with_user.query(DBWorkflow).filter_by(id=test_workflow.id).first()
    )
    assert workflow_in_db is None


def test_delete_workflow_with_incorrect_user(db_with_user: Session):
    client = TestClient(app)
